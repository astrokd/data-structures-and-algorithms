...
const mapTwoToThe = (arr) => {
  return arr.map( function(n,i) {return Math.pow(2,n);});
};
...
const extractStats = (arr) => {
  let abilityArr = arr.map(obj => {
    let aObj = {};
    aObj['name'] = obj.stat.name;
    aObj['total'] = obj.effort + obj.baseStat;
    return aObj;
  });
  return abilityArr;
};
