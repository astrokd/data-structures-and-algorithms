...
const returnNames = (arr) => {
  let names = arr.reduce( (acc,value,idx) => {
    acc[idx] = value.name;
    return acc;
  }, []);
  return names;
...
const extractChildren = (arr) => {
  let hasCharaInName = arr.filter( data => (data.name.includes('a')));
  let children = hasCharaInName.reduce( (accum,val) => {
